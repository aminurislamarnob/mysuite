...
class ExpenseRepository {
  ...
  Stream<List<Account>> watchAccounts() =>
      (_db.select(_db.accounts)..where((t) => t.isArchived.equals(false)))
          .watch();

  Future<List<Account>> accounts() =>
      (_db.select(_db.accounts)..where((t) => t.isArchived.equals(false))).get();

  Future<int> createAccount(
          String name, String type, double balance, int color) =>
      _db.into(_db.accounts).insert(AccountsCompanion.insert(
            name: name,
            type: Value(type),
  ...
  Future<int> createCategory(String name, String icon, int color,
          {bool isIncome = false}) =>
      _db.into(_db.expenseCategories).insert(ExpenseCategoriesCompanion.insert(
            name: name,
            icon: Value(icon),
            color: Value(color),
  ...
  Future<void> deleteTransaction(int id) async {
    await _db.transaction(() async {
      final tx = await (_db.select(_db.expenses)..where((t) => t.id.equals(id)))
          .getSingleOrNull();
      if (tx == null) return;

      await _applyBalance(
          tx.accountId, tx.kind == TxKind.income ? -tx.amount : tx.amount);
  ...
  Future<int> setBudget(
      {required double amount, int? categoryId, String period = 'monthly'}) async {
    ...
    if (existing != null) {
      await (_db.update(_db.budgets)..where((t) => t.id.equals(existing.id)))
          .write(BudgetsCompanion(
              amount: Value(amount), period: Value(period)));
      return existing.id;
    }
    return _db.into(_db.budgets).insert(BudgetsCompanion.insert(
          amount: amount,
          categoryId: Value(categoryId),
          period: Value(period),
        ));
