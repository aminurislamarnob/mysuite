...
class FocusRepository {
  ...
  Future<List<FocusSession>> allSessions() => (_db.select(_db.focusSessions)
        ..where((t) => t.isBreak.equals(false)))
      .get();
  ...
  Future<void> finishSession(
    ...
    int? rating,
      ...
      (_db.update(_db.focusSessions)..where((t) => t.id.equals(id)))
  ...
  Future<Duration> focusedOn(DateTime day) async {
    ...
    final sessions =
        await sessionsBetween(start, start.add(const Duration(days: 1)));
    return Duration(
        seconds: sessions.fold(0, (a, s) => a + s.actualSeconds));
  }
}
