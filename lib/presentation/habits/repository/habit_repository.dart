...
class HabitRepository {
  ...
  Stream<List<HabitLog>> watchLogsForDay(DateTime day) {
    ...
    return (_db.select(_db.habitLogs)..where((t) => t.date.equals(date)))
        .watch();
  }
  ...
  Future<void> setDayAmount(int habitId, DateTime day, double amount,
      {String? note}) async {
    final date = Fmt.dateOnly(day);
    await _db.transaction(() async {
      final existing = await (_db.select(_db.habitLogs)
  ...
  Future<List<Habit>> habitsWithReminders() =>
      (_db.select(_db.habits)
            ..where((t) =>
                t.reminderMinutes.isNotNull() & t.isArchived.equals(false)))
          .get();
  ...
  static const presets = <({
    ...
    (name: 'Water', icon: 'water', color: 0xFF3AAFB9, unit: 'glasses',
        goalType: 0, target: 8, caffeine: null),
    (name: 'Coffee', icon: 'coffee', color: 0xFFF2A03D, unit: 'cups',
        goalType: 1, target: 2, caffeine: 95),
    (name: 'Tea', icon: 'tea', color: 0xFF3BB273, unit: 'cups',
        goalType: 0, target: 2, caffeine: 47),
    (name: 'Exercise', icon: 'exercise', color: 0xFFE5484D, unit: 'minutes',
        goalType: 0, target: 30, caffeine: null),
    (name: 'Reading', icon: 'reading', color: 0xFF9A6DD7, unit: 'pages',
        goalType: 0, target: 20, caffeine: null),
    (name: 'Meditation', icon: 'meditation', color: 0xFF5B7CE0,
        unit: 'minutes', goalType: 0, target: 10, caffeine: null),
    (name: 'Smoking', icon: 'smoking', color: 0xFF6C6C6C, unit: 'cigarettes',
        goalType: 1, target: 0, caffeine: null),
  ];
}
