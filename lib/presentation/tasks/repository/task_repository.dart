...
class TaskRepository {
  ...
  Stream<List<Task>> watchSubtasks(int parentId) =>
      (_db.select(_db.tasks)..where((t) => t.parentTaskId.equals(parentId)))
          .watch();
  ...
  Future<void> updateTask(
    ...
    return (_db.update(_db.tasks)..where((t) => t.id.equals(id))).write(
      TasksCompanion(
        ...
        hasDueTime:
            hasDueTime == null ? const Value.absent() : Value(hasDueTime),
        ...
        boardStatus:
            boardStatus == null ? const Value.absent() : Value(boardStatus),
  ...
  Future<int?> setCompleted(int id, bool completed) async {
    ...
    return createTask(
      ...
      reminderTime: task.reminderTime == null
          ...
          : next.subtract(
              task.dueDate!.difference(task.reminderTime!),
            ),
  ...
  Future<void> addLoggedMinutes(int taskId, int minutes) async {
    ...
    await (_db.update(_db.tasks)..where((t) => t.id.equals(taskId)))
      ...
      loggedMinutes: Value(task.loggedMinutes + minutes),
    ));
  }
  ...
  Future<void> deleteTask(int id) async {
    await _db.transaction(() async {
      await (_db.delete(_db.tasks)..where((t) => t.parentTaskId.equals(id)))
          .go();
  ...
  Future<List<Task>> tasksWithReminders() => (_db.select(_db.tasks)
        ..where((t) =>
            t.reminderTime.isNotNull() & t.isCompleted.equals(false)))
      .get();

  // --- Projects ------------------------------------------------------------

  Stream<List<Project>> watchProjects() => (_db.select(_db.projects)
        ..orderBy([(t) => OrderingTerm(expression: t.sortOrder)]))
      .watch();

  Future<int> createProject(String name, int color, String icon) =>
      _db.into(_db.projects).insert(ProjectsCompanion.insert(
