...
class MedicineRepository {
  ...
  Future<int> createProfile(String name, String relation, int color) =>
      _db.into(_db.medicineProfiles).insert(MedicineProfilesCompanion.insert(
            name: name,
  ...
  Future<Medicine?> getMedicine(int id) => (_db.select(_db.medicines)
  ...
  Future<void> regenerateSchedule(int medicineId, ScheduleSpec spec) async {
    await _db.transaction(() async {
      await (_db.delete(_db.medicineDoses)
            ..where((t) =>
  ...
  Future<void> _insertDoses(int medicineId, List<DateTime> times) async {
    await _db.batch((b) {
      b.insertAll(
        _db.medicineDoses,
        times.map((t) => MedicineDosesCompanion.insert(
  ...
  Future<void> updateMedicine(int id, MedicinesCompanion companion) =>
      (_db.update(_db.medicines)..where((t) => t.id.equals(id)))
          .write(companion);
  ...
  Future<void> setDoseStatus(int doseId, int status) async {
    await _db.transaction(() async {
      final dose = await (_db.select(_db.medicineDoses)
            ..where((t) => t.id.equals(doseId)))
          .getSingleOrNull();
      if (dose == null) return;

      await (_db.update(_db.medicineDoses)..where((t) => t.id.equals(doseId)))
          .write(MedicineDosesCompanion(
        status: Value(status),
        actionedAt:
            Value(status == DoseStatus.pending ? null : DateTime.now()),
      ));
      ...
      await (_db.update(_db.medicines)..where((t) => t.id.equals(med.id)))
  ...
  Stream<List<SymptomLog>> watchSymptoms() => (_db.select(_db.symptomLogs)
        ..orderBy([(t) =>
  ...
  Future<int> logSymptom({
    ...
    String? note,
      ...
      _db.into(_db.symptomLogs).insert(SymptomLogsCompanion.insert(
            symptom: symptom,
  ...
  static ScheduleSpec specOf(Medicine m) => ScheduleSpec(
        ...
        frequency: MedFrequency.values[
            m.frequencyType.clamp(0, MedFrequency.values.length - 1)],
