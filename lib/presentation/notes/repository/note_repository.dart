...
class NoteRepository {
  ...
  Stream<List<Note>> watchNotes({
    ...
    return q.watch().asyncMap((notes) async {
      final links = await (_db.select(_db.noteTags)
  ...
  Future<void> moveToTrash(int id) =>
      (_db.update(_db.notes)..where((t) => t.id.equals(id)))
  ...
  Stream<List<Folder>> watchFolders() =>
      (_db.select(_db.folders)..orderBy([(t) => OrderingTerm(expression: t.name)]))
          .watch();

  Future<int> createFolder(String name, {int? parentId}) =>
      _db.into(_db.folders).insert(
          FoldersCompanion.insert(name: name, parentId: Value(parentId)));

  Future<void> renameFolder(int id, String name) =>
      (_db.update(_db.folders)..where((t) => t.id.equals(id)))
  ...
  Stream<List<Tag>> watchTags() =>
      (_db.select(_db.tags)..orderBy([(t) => OrderingTerm(expression: t.name)]))
          .watch();
