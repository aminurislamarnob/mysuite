...
class ExportService {
  ...
  Future<File> expensesCsv({DateTime? from, DateTime? to}) async {
    ...
    final categories = {
      for (final c in await _db.select(_db.expenseCategories).get()) c.id: c.name
    };
    final accounts = {
    ...
    final rows = <List<dynamic>>[
      ...
      ...filtered.map((e) => [
            Fmt.iso(e.date),
            switch (e.kind) { 1 => 'Income', 2 => 'Transfer', _ => 'Expense' },
            e.amount,
    ...
    final rows = <List<dynamic>>[
      ...
      ...(doses..sort((a, b) => a.scheduledTime.compareTo(b.scheduledTime)))
        ...
        return [
          ...
          switch (d.status) { 1 => 'Taken', 2 => 'Skipped', _ => 'Pending' },
        ];
      }),
    ];
  ...
  Future<File> habitsCsv() async {
    ...
    final rows = <List<dynamic>>[
      ...
      ...(logs..sort((a, b) => a.date.compareTo(b.date))).map((l) => [
            Fmt.iso(l.date),
  ...
  Future<File> fullJsonBackup() async {
    final data = <String, dynamic>{
      ...
      'categories':
          (await _db.select(_db.expenseCategories).get()).map(_json).toList(),
      'expenses': (await _db.select(_db.expenses).get()).map(_json).toList(),
      'budgets': (await _db.select(_db.budgets).get()).map(_json).toList(),
      'recurringExpenses':
          (await _db.select(_db.recurringExpenses).get()).map(_json).toList(),
      'medicineProfiles':
          (await _db.select(_db.medicineProfiles).get()).map(_json).toList(),
      'medicines': (await _db.select(_db.medicines).get()).map(_json).toList(),
      'medicineDoses':
          (await _db.select(_db.medicineDoses).get()).map(_json).toList(),
      'symptomLogs':
          (await _db.select(_db.symptomLogs).get()).map(_json).toList(),
      'focusSessions':
          (await _db.select(_db.focusSessions).get()).map(_json).toList(),
    };
    return _write('mysuite_backup_${Fmt.iso(DateTime.now())}.json',
  ...
  Future<File> medicineSchedulePdf({
    ...
    required List<({String medicine, String dosage, String meal, DateTime at, int status})>
        doses,
    double? adherence,
  }) async {
        ...
        header: (_) => pw.Container(
          ...
          child: pw.Column(
            ...
            children: [
              ...
              pw.Text('Generated ${Fmt.dayMonthYear(DateTime.now())}',
        ...
        build: (_) => [
          for (final day in days) ...[
            ...
            pw.TableHelper.fromTextArray(
              headers: const ['Time', 'Medicine', 'Dosage', 'Instruction', 'Status'],
  ...
  Future<File> expenseReportPdf({
    ...
    required List<({DateTime date, String category, String note, double amount, int kind})>
        rows,
  }) async {
    final doc = pw.Document();
    doc.addPage(
        ...
        build: (_) => [
          ...
          pw.Row(
            ...
            children: [
              ...
              _pdfStat('Expense', '$currency${totalExpense.toStringAsFixed(0)}'),
              _pdfStat('Net',
  ...
  Future<void> shareNoteAsPdf(
    ...
    await _share(await noteAsPdf(title: title, body: body), text: title);
  }
